...
字体由浏览器读取后通过 `fontkit` 转换为 SVG `<path>`。所有中文使用 `jtbz_A.ttf`，所有数字和英文使用 `jtbz_B.ttf`。项目保留其余字体文件，但当前生成流程不使用它们。生成结果不包含 `<text>`，在未安装字体的设备上也能正确显示。
