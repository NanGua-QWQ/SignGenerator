...
export function SignList({ title, signs, selectedId, onSelect, onAdd, addChoices = [], onDelete, onReorder, onUpdate }: SignListProps) {
  ...
  const overSign = (event: DragEvent<HTMLDivElement>, sign: Sign) => {
    ...
    setDropTarget({ id: sign.id, position: dropPosition(event) })
  }
      ...
      <div className="p-3 max-md:py-2">
        ...
        <div className="flex flex-col gap-1.5 max-md:flex-row max-md:overflow-x-auto">
          {signs.map(sign => {
            ...
            return (
              <div key={sign.id} className={`group relative shrink-0 rounded-md border-y-2 transition-colors max-md:w-40 ${dropClass} ${draggingId === sign.id ? 'opacity-50' : ''} ${sign.id === selectedId ? 'bg-accent text-accent-foreground' : 'bg-muted/50 hover:bg-muted'}`} onContextMenu={event => openEditor(event, sign)} onDragOver={event => overSign(event, sign)} onDrop={event => dropSign(event, sign)}>
                <button type="button" draggable={Boolean(onReorder)} onDragStart={event => startDrag(event, sign)} onDragEnd={endDrag} className="absolute left-1 top-1/2 flex size-6 -translate-y-1/2 cursor-grab items-center justify-center rounded text-muted-foreground opacity-60 hover:bg-accent-foreground/10 hover:text-foreground active:cursor-grabbing group-hover:opacity-100" aria-label={`拖动排序 ${signTitle(sign)}`} title="拖动排序">
