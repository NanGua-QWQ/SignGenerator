...
export function SignList({ title, signs, selectedId, onSelect, onAdd, onDelete }: SignListProps) {
  const signBadge = (sign: Sign) => sign.template === 'road-fork-preview' ? '分岔' : sign.code || 'G15'
  const signTitle = (sign: Sign) => sign.template === 'road-fork-preview' ? sign.name || '道路分岔预告' : sign.name || '高速编号牌'
          ...
          {signs.map(sign => (
            <div key={sign.id} className={`group relative shrink-0 rounded-md transition-colors max-md:w-40 ${sign.id === selectedId ? 'bg-accent text-accent-foreground' : 'bg-muted/50 hover:bg-muted'}`}>
              <button type="button" onClick={() => onSelect(sign.id)} className="flex w-full items-center gap-2 p-2 pr-8 text-left">
                <span className={`inline-flex h-6 min-w-8 items-center justify-center rounded-sm px-1 text-[10px] font-bold text-white ${sign.template === 'road-fork-preview' ? 'bg-[#050203]' : 'bg-[#359b47]'}`}>{signBadge(sign)}</span>
