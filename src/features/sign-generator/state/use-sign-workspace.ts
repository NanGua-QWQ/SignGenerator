...
export function useSignWorkspace() {
  ...
  const changeTab = useCallback(
    (tab: WorkspaceTab) => {
      ...
      setSelectedId(current =>
        signsForTab.some(sign => sign.id === current) ? current : firstSign.id,
      )
    },
    [signs],
  )
  ...
  const deleteSign = useCallback(
    (id: string) => {
      setSigns(current => {
        ...
        if (!target || current.filter(sign => sign.template === target.template).length === 1)
          return current

        const next = current.filter(sign => sign.id !== id)
        if (id === selectedId)
          setSelectedId(next.find(sign => sign.template === target.template)?.id ?? next[0].id)
        return next
      })
    },
    [selectedId],
  ...
  const addChoices =
    activeTab === 'interchange-guidance'
      ? INTERCHANGE_ADD_CHOICES
