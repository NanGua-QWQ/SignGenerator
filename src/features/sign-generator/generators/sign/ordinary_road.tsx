...
const ROAD_CONFIG: Record<OrdinaryRoadKind, OrdinaryRoadConfig> = {
  ...
  'ordinary-provincial': { codePrefix: 'S', label: '省道', border: BLACK, fill: YELLOW, text: BLACK },
...
function cleanDigits(value: string): string {
  return String(value || '').replace(/\D/g, '').slice(0, 3) || '105'
}
...
{/*       <g transform={REFERENCE_PATH_TRANSFORM} opacity={0.45}>
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={WIDTH} height={HEIGHT} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={`${code} ${config.label}标识牌`}>
        <rect x={0} y={0} width={WIDTH} height={HEIGHT} rx={OUTER_RADIUS} fill={config.fill} stroke="none" strokeMiterlimit={10} />
